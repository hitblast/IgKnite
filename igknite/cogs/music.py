...
class YTDLSource(disnake.PCMVolumeTransformer):
    ...
    def parse_duration(duration: int) -> str:
        ...
        durations = []
        if days > 0:
            durations.append(f'{days}d')
        if hours > 0:
            durations.append(f'{hours}h')
        if minutes > 0:
...
class Music(commands.Cog):
    ...
    async def _play(
        ...
        if (
            ...
            'https://open.spotify.com/album/' in keyword or 'spotify:album:' in keyword
        ):
            ...
            'https://open.spotify.com/track/' in keyword or 'spotify:track:' in keyword
        ):
    ...
    @commands.message_command(
        name='Search & Play', contexts=disnake.InteractionContextTypes(guild=True)
    )
    ...
    @commands.user_command(
        name='Rich Play', contexts=disnake.InteractionContextTypes(guild=True)
    )
